...
def SNN(d, lR, itr):
    ...
    while i < itr:
        ...
        for row in d:
            ...
            net_output = (
                sA_h1 * w_h_o[0] + sA_h2 * w_h_o[1] + sA_h3 * w_h_o[2] + 1 * w_bias_[3]
            )
            sigmoidActivation_output = sigmoidActivation(net_output)  # net output

            error = (
                row[2] - sigmoidActivation_output
            )  # Error in output and actual prediction

            if error:
                # Calculating error at hidden layer
                delta_o = (
                    sigmoidActivation_output
                    * (1 - sigmoidActivation_output)
                    * (row[2] - sigmoidActivation_output)
                )
                ...
                # Updating the weights at output layer
